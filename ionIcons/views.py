...
def IIicons(request):
    icons = ion_icons.objects.all()
    paginator = Paginator(icons, 100)
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    context = {"icon": icons, "page_obj": page_obj}
    return render(request, "ionIcons/ionIcons.html", context)


def IIiconsPagination(request):
    icons = ion_icons.objects.all()
...
@cache_page(60 * 2) # caches page for 2 mins interval
def IIiconsSearch(request):
    query = request.GET.get('search')
    search_terms = query.split() if query else []
    allSearchTag = ion_icons.objects.all()
...
# This code Iterate my json file to data base in seconds

from django.db import transaction

def import_icons(request):
    with open('D:/Django/IconPort/static/ion-icons.json') as f:
        data = json.load(f)
        
        with transaction.atomic():
            ion_icons.objects.all().delete()  # Optional: Clear existing data before importing
            ...
            ion_icons.objects.bulk_create(objs)
            
    return render(request, "ionIcons/ionIcons.html")
